...
//     depth_first_pre_instr_cnt: Vec<usize>, // 存储各基本块按深度优先顺序后的pre_instr_cnt
...
//         let mut depth_first_pre_instr_cnt = Vec::new();
//         depth_first_pre_instr_cnt.resize(depth_first_order.len(), 0);
//         for i in 1..depth_first_order.len() {
//             let last_bb = target.blocks.get(depth_first_order[i-1]).unwrap();
//             depth_first_pre_instr_cnt[i] = depth_first_pre_instr_cnt[i-1] + last_bb.instrs.len();
//         }
...
//         (0..instr_len).map(|i|{
//             let res = (this_bb.pre_instr_cnt+i, *cnt);
//             *cnt += 1;
//             res
//         })
//         .collect()
...
//             func.blocks.iter()
//                 .for_each(|b|
//                     println!("{}: pre_instr_cnt={}, self_instr={}",
//                         b.label,
//                         b.pre_instr_cnt,
//                         b.instrs.len()
//                     )
...
//                 let block_from = self.depth_first_pre_instr_cnt.get(depth_idx).unwrap();
